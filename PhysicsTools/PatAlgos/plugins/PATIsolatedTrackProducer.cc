...
namespace pat {
  ...
  public:
    ...
    void getAssociatedRecHits(const PolarLorentzVector&,
                              const EBRecHitCollection&,
                              const EERecHitCollection&,
                              const HBHERecHitCollection&,
                              EBRecHitCollection&,
...
void pat::PATIsolatedTrackProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned int igt = 0; igt < generalTracks->size(); igt++) {
    ...
    getAssociatedRecHits(polarP4, *EBRecHits, *EERecHits, *HBHERecHits, associatedEBRecHits, associatedEERecHits, associatedHBHERecHits);
...
void pat::PATIsolatedTrackProducer::getAssociatedRecHits(const PolarLorentzVector& p4,
                                                         const EBRecHitCollection& EBRecHits,
                                                         const EERecHitCollection& EERecHits,
                                                         const HBHERecHitCollection& HBHERecHits,
                                                         EBRecHitCollection& AssociatedEBRecHits,
                                                         EERecHitCollection& AssociatedEERecHits,
                                                         HBHERecHitCollection& AssociatedHBHERecHits) const {
  for (const auto& hit : EBRecHits) {
    if (insideCone(p4, hit.detid(), associatedCaloEnergy_DR_))
      AssociatedEBRecHits.push_back(hit);
  }
  for (const auto& hit : EERecHits) {
    if (insideCone(p4, hit.detid(), associatedCaloEnergy_DR_))
      AssociatedEERecHits.push_back(hit);
  }

  for (const auto& hit : HBHERecHits) {
    if (insideCone(p4, hit.detid(), associatedCaloEnergy_DR_))
      AssociatedHBHERecHits.push_back(hit);
  }
}
